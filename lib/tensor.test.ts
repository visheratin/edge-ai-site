...
test("2D array", () => {
  const tensor = new Tensor(
    new OrtTensor(new Float32Array([1, 2, 3, 4, 5, 6, 7, 8, 9]), [3, 3])
  );
...
test("1D argmax iterator", () => {
  const input = new Tensor(
    new OrtTensor(new Float32Array([1, 2, 3, 4, 5, 6, 7, 8, 9]), [9])
  );
...
test("2D argmax iterator", () => {
  ...
  let maxValues = new Tensor(
    new OrtTensor(new Float32Array([-Infinity, -Infinity, -Infinity]), [3])
  );
...
test("3D argmax iterator", () => {
  const input = new Tensor(
    new OrtTensor(
      new Float32Array([1, 9, 10, 11, 15, 16, 2, 3, 12, 13, 14, 4, 5, 6, 7, 8]),
      [2, 2, 4]
    )
  );
  const output = new Tensor(
    new OrtTensor(new Float32Array([3, 1, 2, 3]), [2, 2])
  );
  let argmax = new Tensor(
    new OrtTensor(new Float32Array([0, 0, 0, 0]), [2, 2])
  );
  let maxValues = new Tensor(
    new OrtTensor(
      new Float32Array([-Infinity, -Infinity, -Infinity, -Infinity]),
      [2, 2]
    )
  );
...
test("3D argmax iterator", () => {
  ...
  expect(argmax.ortTensor).toEqual(output.ortTensor);
  output = new Tensor(
    new OrtTensor(new Float32Array([1, 1, 0, 0, 0, 0, 0, 1]), [2, 4])
  );
